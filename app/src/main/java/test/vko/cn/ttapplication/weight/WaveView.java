...
//				mLevelLine = 0;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasured) {
            isMeasured = true;
            mViewHeight = getMeasuredHeight() + getUpDownDistance();
            mViewWidth = getMeasuredWidth();
            // 水位线从最底下开始上升
            mLevelLine = mViewHeight / 2;
            // 根据View宽度计算波形峰值
            mWaveHeight = mViewWidth / 9.5f;
            // 波长等于四倍View宽度也就是View中只能看到四分之一个波形，这样可以使起伏更明显
            mWaveWidth = mViewWidth * 1.05f;
            // 左边隐藏的距离预留一个波形
            mLeftSide = -mWaveWidth;
...
//switch (currentIndex){
//    case 0:
//        moveToY= mPointsList.get(0).getY();
//        break;
//    case 1:
//        moveToY= mPointsList.get(1).getY();
//        break;
//    case 2:
//        moveToY= mPointsList.get(2).getY();
//        break;
//    case 3:
//        moveToY= mPointsList.get(3).getY();
//        break;
//
//}
